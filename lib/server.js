...
module.exports = function(app, sq){
  ...
  function createServerForm(req, res){
    ...
    if (req.xhr)
      res.partial('server/new', {server: server})
    else
    ...
    server.save(function(err){
      if (!err) {
        ...
        sq.lib.async.forEachSeries(serviceChanges, updateService, function(err){
          sq.Server.findById(server.id, function(err, updatedServer) { //Reload the updated server object
            sq.debug(updatedServer, "CHECK THIS SERVER")
            checkServerServices(updatedServer, function(){
              res.redirect('/')
            })
          })
        })
