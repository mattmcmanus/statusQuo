...
module.exports = function(app, sq){
  ...
  sq.io.sockets.on('connection', function(socket){
    sq.jobs[socket.id] = {}
    sq.debug("socket connection")
    socket.on('ping', function(ip, fn){
      // Set things up
      var buffer = []
      , spawn = require('child_process').spawn
      //, pattern = /(\d+?) bytes from (.+?): icmp_req=(\d+?) ttl=(\d+?) time=(.+) ms/ // Linux Output
      , pattern = /(\d+?) bytes from (.+?): icmp_seq=(\d+?) ttl=(\d+?) time=(.+) ms/ // Mac Output
      , output

      sq.jobs[socket.id].ping = spawn('ping', [ip])
      var ping = sq.jobs[socket.id].ping
      ...
      ping.stdout.on('data', function (data) {
        ...
        socket.json.emit('response', output)
      });
    ...
    socket.on('disconnect', function () {
      if (typeof sq.jobs[socket.id] !== "undefined") sq.jobs[socket.id].ping.kill()
    });
  });
};
